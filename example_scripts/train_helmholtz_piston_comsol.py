...
from pinnlib.data_utils.point_cloud import PointCloud, filter_points, sample_points
...
piston_velocity = meta_data["piston_velocity"]
wave_speed = meta_data["wave_speed"]
...
lower_bound = 1e-5  # lower bound for hom pde loss

data_generator = DataPointGenerator(
    point_cloud=sample_points(subsample_key, 128)(pressure_pc),
    batch_size=128,
    key=data_key,
)
...
mesh_generator = MeshGenerator(mesh, batch_size=128, key=bnd_key)


# Filter evaluation data on surface plane
z_filter = filter_points(lambda c, _: c[-1] < 1e-12)
eval_pressure_pc = z_filter(pressure_pc)
eval_velocity_pc = z_filter(velocity_pc)
...
def boundary_velocity_loss(
    ...
    batched_fwd_model = jax.vmap(fwd_model.v_net, in_axes=(None, *[0] * n_dim * 2))
...
def compute_pressure(params, pinn):
    ...
    return p(params, *eval_pressure_pc.coords.T, 0.0)


@eqx.filter_jit
def compute_velocity(params, pinn):
    """Evaluates pressure over the same grid as original dataset"""
    v = grid_map(pinn.v_net, axis_mask=(0, 1, 1, 0, 0, 0, 0))
    return v(params, *eval_pressure_pc.coords.T, 0.0, 0.0, 0.0, 1.0)


def plot_pred(pressure):
    fig, ax = plt.subplots()
    pc = ax.scatter(
        *eval_pressure_pc.coords.T,
        c=pressure,
        cmap="plasma",
    )
    ax.set_xlabel("X")
    ...
    ax.plot(x, y, "w--", linewidth=2)  # 'w--' = white dashed line
...
infinite_disk_loader = iter(source_plane_generator)
...
velocity_model = (
    lambda params, *x: params if jnp.array(x).sum() ** 2 <= piston_radius**2 else 0.0
)
velocity_params = jnp.array(piston_velocity)
...
for step, data_batch, pde_batch, bnd_batch in tqdm(
    ...
    if step % log_every == 0:
        ...
        logger.log_scalar("inv/velocity-error", velocity_params - piston_velocity, step)  # pyright: ignore
        ...
        # make a prediction
        p_pred = compute_pressure(pinn_params, pinn)
        v_pred = compute_velocity(pinn_params, pinn)
        p_pred_error = split_real_and_imaginary_metric(aggregated_metrics["mse"])(
            p_pred, eval_pressure_pc.vals
        )

        logger.log_scalar("errors/mse_re", p_pred_error.real, step)
        logger.log_scalar("errors/mse_im", p_pred_error.imag, step)
        ...
        error = split_real_and_imaginary_metric(sq_error)(p_pred, eval_pressure_pc.vals)
