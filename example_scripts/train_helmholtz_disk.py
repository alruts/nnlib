...
# Simulate a rigid piston using the Rayleigh integral method
...
mesh = trimesh.load_mesh("data/disk_01.stl")
disk_generator = MeshGenerator(mesh, batch_size=128)
...
def compute_velocity(params, pinn):
    ...
    return v(params, *eval_data.coordinate_arrays, 0.0, 0.0, 0.0, 1.0)
...
def plot_pred(pressure):
    ...
    x = disk_in_baffle.disk_radius * jnp.cos(theta)
    y = disk_in_baffle.disk_radius * jnp.sin(theta)
...
infinite_disk_loader = iter(disk_generator)

# initialize logger
logger = TensorboardLogger(experiment_name=f"test-run_{len(data_generator)}")
logger.log_plot("plots/p_gt_mag", plot_pred, jnp.abs(eval_data.vals), 0)
logger.log_plot("plots/p_gt_phase", plot_pred, jnp.angle(eval_data.vals), 0)
log_every = 1000
...
for step, data_batch, pde_batch, bnd_batch in tqdm(
    ...
    if step % log_every == 0:
        ...
        error = split_real_and_imaginary_metric(sq_error)(p_pred, eval_data.vals)
