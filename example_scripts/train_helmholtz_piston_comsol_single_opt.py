...
data_pipe = pc_utils.pipe(
    pc_utils.filter_points(lambda c, _: c[-1] <= 0.25 * wavelength),
    pc_utils.filter_points(lambda c, _: c[-1] > 1e-2),
    pc_utils.sample_points(subsample_key, 1024),
)
...
params = (velocity_params, pinn_params)
...
@eqx.filter_jit
def train_step(model, inv_model, params, opt_state, weights, batch):
    (total, each_term), grads = jax.value_and_grad(
        lambda p: compute_weighted_loss(
            p[1],
            model=model,
            batch=batch,
            weights=weights,
...
for step, data_batch, pde_batch, bnd_batch in tqdm(
    ...
    # # smoothed updates to velocity params
    # _velocity_params, pinn_params = params
    # velocity_params = running_average(0.9, velocity_params, _velocity_params)
    # params = (velocity_params, pinn_params)
    ...
    if step % update_weights_every == 0:
        ...
        loss_weights = running_average(0.9, loss_weights, new_weights)
    ...
    loss_weights["bnd"] *= compute_mask(filtered_loss_sum, 1.0, b, 1.0)  # pyright: ignore

    if step % log_every == 0:
        velocity_params, pinn_params = params
        logger.log_scalar("loss/total", loss, step)
        logger.log_scalar(
            "inv/percent-relative-velocity-error",
            100 * (jnp.abs(velocity_params[0] - piston_velocity) / piston_velocity),
            step,
        )

        logger.log_scalar(
            "inv/v_pred-minus-v_gt",
            velocity_params[0] - piston_velocity,
            step,
        )
        ...
        # what the 'velocity model' looks like
